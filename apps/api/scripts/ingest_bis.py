"""Live BIS import through the full ingestion path.

Fetches mapped indicator(s) via BISAdapter, persists the DTOs, and records one
IngestionRun per country × indicator — each in its own caller-owned
transaction (committed only if that run succeeds).

    python scripts/ingest_bis.py --country CHE --all-mapped --start 2000 --end 2025
    python scripts/ingest_bis.py --all-countries --all-mapped --start 2000 --end 2025
...
from app.data_sources.bis_mappings import BIS_MAPPINGS, get_bis_mapping
...
async def import_one(adapter: BaseDataSourceAdapter, country: str, indicator_code: str,
    ...
    mapping = get_bis_mapping(indicator_code)
    if mapping is None:
        print(f"[{country} / {indicator_code}] No BIS mapping — skipped.")
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="BIS live import")
    ...
    parser.add_argument("--all-mapped", action="store_true",
                        help="Import every indicator in the central BIS mapping registry")
    ...
    if args.all_mapped:
        indicator_codes = [m.indicator_code for m in BIS_MAPPINGS]
    elif args.indicator:
        indicator_codes = [args.indicator]
    else:
        indicator_codes = ["CREDIT_TO_GDP_GAP"]

    span = f"{args.start}–{args.end or 'now'}"
    print(f"BIS batch import: {len(countries)} countries × "
    ...
    async with BISAdapter() as bis:
        adapter = TransientRetryAdapter(bis)
