...
Owner-directed, timeboxed METHODOLOGY/ARCHITECTURE brief — resolve the architecture required before the first confidence implementation, with NO confidence numbers, NO uncertainty ingestion, NO force aggregation, and NO migration executed. Four questions: (1) what belongs to INDICATOR confidence; (2) what belongs to FORCE confidence; (3) which WGI uncertainty data Atlas should consume; (4) how it should be stored without corrupting canonical indicator/force/coverage semantics. Model version stays normalization-v0.6 unless an executable output changes (none did).

### Completed

- **Layering resolved (NORMALIZATION.md §10 rewritten → 10.1/10.2/10.3)**: 10.1 permanent rules — confidence independent from strength (economic scores NEVER multiplied by confidence; level 90 + confidence 0.4 must never become 36), confidence ≠ freshness (freshness_factor REUSED, never recomputed, never equated), missing uncertainty ≠ perfect ≠ zero (unknown components stay None; the score itself stays usable), no arbitrary numeric provider-quality constants (qualitative categories may be TYPED later — official_primary / official_republished / proxy_measure / perception_composite). 10.2 INDICATOR confidence — source_quality (qualitative only), freshness_factor, optional measurement_uncertainty + method_sufficiency diagnostics (DSR sample_n/min_n/span already carried as OwnHistoryLevelResult provenance; credit gap is parametric — no Atlas history sample; no "20 obs = 0.6" conversions approved); WGI same-country/dimension/SOURCE-PERIOD uncertainty alignment rule. 10.3 FORCE confidence — coverage/completeness, missing inputs, proxy completeness and DEC-009 ceilings are FORCE-LAYER ONLY: a high-quality fresh POLITICAL_STABILITY_WGI_SCORE signal is never lowered because Internal conflict is PARTIAL-ceiling-capped; SIPRI-spending-as-input belongs to Military-strength force completeness, not a fake low indicator source-quality number. No ForceConfidence implemented.
- **WGI uncertainty audit (live read-only probe, temporary script deleted after the run)**: exact series verified against WB API v2 metadata — score `GOV_WGI_{RL,CC,PV}_SC` (WDI source 2) + `GOV_WGI_{dim}.SC_LB` / `.SC_UB` (90% CI bounds FOR THE GOVERNANCE SCORE — dedicated WGI source id 3) / `.SE` (standard error OF THE ESTIMATE) / `.SR` (number of sources). All 8 tracked countries × all 4 series × every score year 1996–2024 (208 obs per series = 8 × 26; biennial 1997/1999/2001 gaps only; no 2025 values yet) with PERFECT one-to-one year alignment to the score series. Scale verified: LB ≤ score ≤ UB for all 624 points — bounds on the SAME 0–100 score scale (symmetric; UB clamps at 100, e.g. CHE CC 2020); SE on the underlying estimate scale (~0.15–0.25) with empirically width ≈ 56.5 × SE — NOT 65.8 × SE — so SE CANNOT reconstruct the published bounds; SR integer counts 4–16. Sample 2024 CHE RL: 87.32 / 82.04 / 92.60 / width 10.56 / SE 0.1871 / SR 10.
- **Initial WGI uncertainty input set selected: LB + UB + SR** (brief's Option B) — CI width = UB − LB is the primary score-scale measurement diagnostic (no scale mixing), SR a distinct data-richness diagnostic; SE DEFERRED (estimate-scale, non-reconstructive). NO CI-width or source-count → confidence curve designed or approved; no confidence number exists.
- **Storage architecture decision matrix (NORMALIZATION.md "Sprint 5.13")**: Option A auxiliary canonical indicators REJECTED (would appear in /api/indicators as economic indicators, inflate the per-country indicator_count (distinct sourced indicators), add catalog rows the force-coverage aggregate reads, "25 → 34 indicators" semantic confusion, uncertainty masquerading as force inputs). Option B multiple SourceSeries under the existing WGI canonical indicator INVALID (align_observation_as_of / own_history_as_of select by indicator; two active series on the same period → alignment could nondeterministically return a bound instead of the score; the WB adapter's canonical mapping would store bound values as score observations). Option D raw_payload provider metadata FACTUALLY UNAVAILABLE (the WB score-series records carry no uncertainty fields — the uncertainty series are separate source-3 series; verified in the adapter's record handling). Option C DEDICATED auxiliary-diagnostics storage CHOSEN: a new `indicator_diagnostics` table keyed to the BASE canonical indicator (country, diagnostic kind ci_lower_bound/ci_upper_bound/source_count, provider series code, period, value, retrieved-at, vintage) — catalog/counts/coverage/registry/alignment all untouched, raw provider values with revision handling, extensible beyond WGI; deliberately NO SourceSeries rows (a SourceSeries on the base indicator is one careless ingest away from Option B's failure). REQUIRES AN ALEMBIC MIGRATION → STOPPED BEFORE IMPLEMENTATION per the brief.
- **Sprint 5.14 spec recorded**: (1) indicator_diagnostics migration; (2) WB fetch path for the 3 dims × SC_LB/.SC_UB/.SR persisting ONLY into the diagnostics table (never persist_observations), idempotent + revision-aware, IngestionRun-recorded; (3) derived-layer lookup helper — diagnostics at the ALIGNED source period, latest vintage, missing → None; (4) nothing else (no confidence formula, no NormalizedSignal change, no version bump, no public API).
- **Docs**: NORMALIZATION.md — new "Sprint 5.13 methodology status" section (audit + matrix + 5.14 spec), §10 fully rewritten, §17 items 6 (numeric composition only remains open) and 9 (RESOLVED — audited, input set + representation chosen, ingestion = 5.14) updated. DECISIONS.md DEC-022. DATA_SOURCES.md WGI-uncertainty line updated with the verified facts. PROJECT_STATUS / BACKLOG / HANDOFF / DEVLOG updated.

### Results

- pytest: **318 passed, 0 failed** — NO production code changed; the suite re-verified the unchanged Sprint 5.12 baseline (WGI level/momentum/relative, DSR level, credit-gap level unchanged; confidence None everywhere; no force scores; no raw mutations; all offline). Model version stays **normalization-v0.6**.
- Live probe read-only (no DB access, no writes): the uncertainty-audit facts above. The temporary probe script (apps/api/wgi_uncertainty_probe_tmp.py) was deleted after the run.
- No frontend change → no npm build; no API route, model, migration, mapping, seed, or raw-data changes; nothing ingested.

### Not done (deliberately)

- NO confidence 0..1, NO confidence weights, NO source-quality numeric constants, NO CI-width/source-count → confidence curves, NO ForceConfidence, NO force aggregation/weights/Indebtedness score, NO Gini/productivity normalization, NO DSR or credit-gap momentum, NO non-WGI relative, NO phase, NO persistence, NO public normalized API, NO frontend confidence UI — and NO migration (STOPPED before the indicator_diagnostics table; 5.14 spec recorded instead).

### Next

- Sprint 5.14 — owner to choose: the DEC-022 recommendation (indicator_diagnostics migration + WGI LB/UB/SR ingestion + lookup helper), Gini calibration universe, productivity expanded calibration universe, or data-side work (education Option C / IMF WEO / WID).

### Goal

The owner supplied the approved numeric curve for CREDIT_TO_GDP_GAP (the DEC-020 owner-approval list), implementing the Sprint 5.11 verdict: flat 50 at/below +2pp (no-excess region deliberately NEUTRAL, not 100 — absence of excess credit is not evidence of strength), strictly linear from (+2, 50) to (+10, 0) (slope −6.25 per pp), clamped 0 at/above +10pp; negative gaps carry NO penalty (DEC-020 retraction preserved). All four parameters are Atlas MODEL PARAMETERS versioned in the model config; the +2/+10 breakpoints coincide with the Basel CCyB guide L/H but the 50/0 mapping is an Atlas choice (three threshold concepts stay separated). No minimum-history gate (parametric curve); freshness gates the current observation only, never scales; relative stays CONTEXTUAL_DEFERRED, momentum windows (4q/8q) stay UNAPPROVED, confidence None, backtest_safe False; execution gated by an explicit `one_sided_vulnerability_configs` entry for EXACTLY CREDIT_TO_GDP_GAP (registry family alone never auto-enables — same pattern as DSR); model version → normalization-v0.6; DEC-021; no persistence, no public API, no force scores.

### Completed

- **normalization_definitions.py**: `OneSidedVulnerabilityConfig` (neutral_ceiling, saturation_value, no_excess_score, saturated_score; post-init: saturation > ceiling, scores in 0–100, stress must not fall above the ceiling) + `OneSidedVulnerabilityResult` provenance (curve params + level_score, config re-validated) frozen dataclasses; `NormalizedSignal.one_sided_vulnerability_level`; `ModelVersionConfig.one_sided_vulnerability_configs`; `CURRENT_MODEL_VERSION` → **normalization-v0.6** / `sprint-5.12-credit-gap-one-sided-level-r1` with the owner curve `(2.0, 10.0, 50.0, 0.0)` and a full version-history comment; `validate_normalization_registry()` gate (config entry requires the matching registry family); CREDIT_TO_GDP_GAP registry notes updated to implemented.
- **normalizer.py**: dispatcher routes `one_sided_vulnerability` → `_normalize_one_sided_vulnerability_as_of` (config gate → align_observation_as_of → None if no eligible observation → freshness gate → curve); not-implemented error message lists the new executable set. New pure function `one_sided_vulnerability_level(value, config)`: flat no_excess_score at/below the ceiling, linear between, clamped at saturation. Module docstring documents the sprint.
- **Smoke** (`scripts/normalize_smoke.py`): `_print_signal` one_sided_vulnerability branch (score to 2 decimals + curve-parameter line with the neutral/saturation interpretation) + CREDIT_TO_GDP_GAP Basel-caution note (common reference point, NOT a mechanical rule; relative deferred; momentum unapproved).
- **Tests**: Sprint-5.11 still-unscored regression REPLACED by the Sprint 5.12 suite — owner-table exact regression (all 8 owner points through the full align → freshness → curve path), pure-curve clamps/continuity/config-validation (incl. deep-negative → 50 and >+10 → 0), no-auto-enable execution gate, no-observation → None, too-stale (age 12) → None, freshness-decays-but-never-scales (age 3, factor <1, score unscaled), other-dimensions-stay-None + provenance; model-version test updated to v0.6 + exactly-CREDIT_TO_GDP_GAP config; DSR + relative test files bumped to v0.6 (DSR config asserted unchanged) → **318 total**.
- **Docs**: NORMALIZATION.md — new "Sprint 5.12 implementation status" section (owner table verbatim, locked design decisions, Basel-caution limits, live smoke values, test inventory); §6 ONE_SIDED_VULNERABILITY row marked implemented; §7 CREDIT_TO_GDP_GAP row rewritten; §17 item 2 RESOLVED (DEC-021); Sprint-5.6 "current truth" closer + Sprint-5.5 closing-scope note updated (14 remaining non-WGI indicators raise). DECISIONS.md DEC-021 appended. HANDOFF / PROJECT_STATUS / BACKLOG updated.

### Results

- pytest: **318 passed, 0 failed** (all offline; baseline 312).
- Live read-only smoke (tracked_8, CREDIT_TO_GDP_GAP @2025-Q4, normalization-v0.6): USA −11.54 → 50, CHN −7.69 → 50, CHE −17.04 → 50, DEU −3.96 → 50, FRA −15.11 → 50, GBR −17.82 → 50, JPN +6.78 → **20.10**, IND +1.74 → 50; all freshness 1.0, backtest safe false. DSR live regression: CHE @2025-Q4 → 13.4615 (rank 90.5/104) — unchanged values, model version now v0.6. Descriptive note: 7/8 countries sit in the neutral region at this snapshot; JPN is the only one in the declining region.
- No frontend change → no npm build; no API route, model, migration, mapping, seed, or raw-data changes.

### Not done (deliberately)

- NO DSR momentum/relative, NO Gini/productivity/GDP-growth/CPI/ULC scoring, NO non-WGI relative, NO confidence, NO force weights/Indebtedness force score/cycle composite/phase, NO persistence, NO public scoring API, NO frontend score UI, NO new provider imports, NO further credit-gap curve tuning (the owner table is the settled semantics).

### Next

- Sprint 5.13 — owner to choose: confidence-dimension design (WGI uncertainty import first), Gini calibration universe (global empirical history vs tracked_8 + survey-base comparability), productivity expanded calibration universe (OECD-wide), or data-side work (education Option C / IMF WEO / WID).
